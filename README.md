...
### Chi tiết kiến trúc Microservices với tất cả Components

```
                                  Frontend Clients
┌────────────────────────────────────────────────────────────────────────────────────────────┐
│  ┌───────────┐     ┌─────────────┐          ┌─────────────┐     ┌─────────────┐            │
│  │ Web App   │     │ Mobile App  │          │ Admin Panel │     │ Shipper App │            │
│  │ (React)   │     │(React Native│          │  (Vue.js)   │     │ (Flutter)   │            │
│  │ Port: 3000│     │             │          │ Port: 3001  │     │             │            │
│  └─────┬─────┘     └──────┬──────┘          └──────┬──────┘     └──────┬──────┘            │
└────────┼────────────────────┼──────────────────────┼───────────────────┼───────────────────┘
         │                    │                      │                   │
...
┌─────────────┐      ┌─────────────┐      ┌─────────────┐      ┌─────────────┐     ┌─────────────┐
│User Service │      │Shop Service │      │Product Svc  │      │Cart Service │     │Order Service│
│Port: 8001   │────▶│Port: 8002    │────▶│Port: 8003   │────▶│Port: 8004   │────▶│Port: 8005   │
│gRPC: 9001   │      │gRPC: 9002   │      │gRPC: 9003   │      │gRPC: 9004   │     │gRPC: 9005   │
│             │      │             │      │             │      │             │     │             │
│- Register   │      │- CRUD Shops │      │- Products   │      │- Add Items  │     │- Create     │
│- Login/Auth │      │- Analytics  │      │- Categories │      │- Calculate  │     │- Track      │
│- Profile    │      │- Promotions │      │- Inventory  │      │- Coupons    │     │- Status     │
│- Addresses  │      │- Reports    │      │- Pricing    │      │- Persist    │     │- History    │
└─────┬───────┘      └─────┬───────┘      └─────┬───────┘      └─────┬───────┘     └─────┬───────┘
      │                    │                    │                   │                   │
      │                    │                    │                   │                   │
┌─────▼─────┐        ┌─────▼─────┐        ┌─────▼─────┐       ┌─────▼─────┐       ┌─────▼─────┐
│PostgreSQL │        │PostgreSQL │        │PostgreSQL │       │   Redis   │       │PostgreSQL │
│users_db   │        │shops_db   │        │products_db│       │ cart_data │       │orders_db  │
│Port: 5432 │        │Port: 5432 │        │Port: 5432 │       │Port: 6379 │       │Port: 5432 │
└───────────┘        └───────────┘        └───────────┘       └───────────┘       └───────────┘
                                              │
                                        ┌─────▼─────┐
                                        │Elasticsearch│
...
┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│Payment Svc  │     │Shipping Svc │     │Review Svc   │
│Port: 8006   │     │Port: 8009   │     │Port: 8012   │
│gRPC: 9006   │     │gRPC: 9009   │     │gRPC: 9012   │
│             │     │             │     │             │
│- Gateways   │     │- Shippers   │     │- Reviews    │
│- Refunds    │     │- Tracking   │     │- Ratings    │
│- Escrow     │     │- Geo/Maps   │     │- Media      │
│- History    │     │- Calculate  │     │- Moderate   │
└─────┬───────┘     └─────┬───────┘     └─────┬───────┘
      │                   │                   │
┌─────▼─────┐       ┌─────▼─────┐       ┌─────▼─────┐
│PostgreSQL │       │PostgreSQL│       │PostgreSQL│
│payments_db│       │shipping_db│       │reviews_db │
...
┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│Search/Rec   │     │Notification │     │Media Service│     │Analytics    │
│Service      │     │Service      │     │Port: 8010   │     │Service      │
│Port: 8007   │     │Port: 8008   │     │gRPC: 9010   │     │Port: 8011   │
│gRPC: 9007   │     │gRPC: 9008   │     │             │     │gRPC: 9011   │
│             │     │             │     │- Upload     │     │             │
│- Search     │     │- Push/Email │     │- Process    │     │- BI Reports │
│- ML Rec     │     │- SMS/WebSoc │     │- Resize     │     │- Metrics    │
│- Trending   │     │- Templates  │     │- CDN        │     │- Tracking   │
│- Behavior   │     │- Prefs      │     │- Storage    │     │- Forecasts  │
└─────┬───────┘     └─────┬───────┘     └─────┬───────┘     └─────┬───────┘
      │                   │                   │                   │
┌─────▼─────┐       ┌─────▼─────┐       ┌─────▼─────┐       ┌─────▼─────┐
│Elasticsearch│     │   Redis   │       │  MongoDB  │       │  MongoDB  │
│+ Redis Cache│     │preferences│       │media_meta │       │analytics  │
│Port: 9200   │     │Port: 6379 │       │Port: 27017│       │Port: 27017│
│Port: 6379   │     └───────────┘       └───────────┘       └───────────┘
└─────────────┘                               │                   │
                                        ┌─────▼─────┐       ┌─────▼─────┐
                                        │AWS S3/MinIO│       │ InfluxDB  │
                                        │file_storage│       │time_series│
                                        │           │       │Port: 8086 │
                                        └───────────┘       └───────────┘
...
═══════════════════════════════════════════════════════════════════════════════════════════════
          ...
          │                             │                             │
┌─────────▼─────────┐         ┌─────────▼─────────┐         ┌─────────▼─────────┐
│   Publishers      │         │   Subscribers     │         │   Broadcast       │
│                   │         │                   │         │                   │
│ • Order Service   │         │ • Payment Service │         │ • Notification    │
│ • Payment Service │         │ • Shipping Service│         │ • Analytics       │
│ • User Service    │         │ • Notification    │         │ • All Services    │
│ • Product Service │         │ • Analytics       │         │   (Events)        │
└───────────────────┘         └───────────────────┘         └───────────────────┘
...
═══════════════════════════════════════════════════════════════════════════════════════════════
                    ...
                    │                   │                   │
          ┌─────────▼─────────┐┌────────▼────────┐┌─────────▼─────────┐
          │    Fast Access    ││   Session Mgmt  ││   Real-time Data  │
          │                  ││                 ││                   │
          │ • Product Catalog││ • User Sessions ││ • Live Tracking   │
          │ • Search Cache   ││ • Auth Tokens   ││ • Notifications   │
          │ • Trending Items ││ • OTP Codes     ││ • Cart Updates    │
          └──────────────────┘└─────────────────┘└───────────────────┘
...
┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│Payment      │     │Maps & Geo   │     │Cloud Storage│     │Communication│
│Gateways     │     │Services     │     │Services     │     │Services     │
│             │     │             │     │             │     │             │
│• Stripe     │────▶│• Google Maps│────▶│• AWS S3     │────▶│• SMTP Server│
│• VNPay      │     │• Mapbox     │     │• MinIO      │     │• SMS Gateway│
│• Momo       │     │• OpenStreet │     │• CloudFront │     │• FCM Push   │
│• PayPal     │     │• Geocoding  │     │• CDN        │     │• WebSocket  │
└─────────────┘     └─────────────┘     └─────────────┘     └─────────────┘
...
│Port: 9090   │     │Port: 3000   │     │• Elasticsearch  │     │Port: 14268 │
│             │     │             │     │• Logstash   │     │             │
│• CPU/Memory │     │• Alerts     │     │• Kibana     │     │• Distributed│
│• API Calls  │     │• Reports    │     │• Log Search │     │• Performance│
│• DB Queries │     │• Real-time  │     │• Analytics  │     │• Debug     │
└─────────────┘     └─────────────┘     └─────────────┘     └─────────────┘
...
1. User Login:
   Client → API Gateway → User Service → PostgreSQL → Redis (JWT) → Response

2. Product Search:
   Client → API Gateway → Search Service → Elasticsearch → Product Service → Redis Cache

3. Add to Cart:
   Client → API Gateway → Cart Service → Redis → Product Service (inventory check)

4. Place Order:
   Cart Service → Order Service → PostgreSQL → RabbitMQ (order.created event)
   
5. Process Payment:
   RabbitMQ → Payment Service → External Gateway → PostgreSQL → RabbitMQ (payment.done)
   
6. Assign Shipping:
   RabbitMQ → Shipping Service → PostgreSQL → Maps API → WebSocket (live tracking)
   
7. Notifications:
   RabbitMQ → Notification Service → SMTP/SMS/Push → WebSocket (real-time)
   
8. Analytics:
   All Events → Analytics Service → MongoDB → InfluxDB → Grafana (reports)
